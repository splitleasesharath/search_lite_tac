...
AVAILABLE_ADW_WORKFLOWS = [
    ...
    "adw_plan_implement_update_task",  # Multi-agent task processing (plan + implement + update)
]
...
def extract_prompt_from_issue_body(text: str, workflow_command: str) -> str:
    """Extract the prompt/task description from issue body by removing workflow metadata.

    Args:
        text: The full issue body text
        workflow_command: The detected workflow command (e.g., 'adw_chore_implement')

    Returns:
        The cleaned prompt text with workflow commands and metadata removed
    """
    # Remove common patterns that are not part of the actual prompt
    lines = text.split('\n')
    cleaned_lines = []
    ...
    for line in lines:
        ...
        # Skip issue type markers (/feature, /bug, /chore)
        if line_lower in ['/feature', '/bug', '/chore']:
            continue

        # Skip workflow command lines
        if workflow_command.lower() in line_lower and len(line_lower) < 50:
            continue

        # Skip ADW ID lines
        if 'adw_id' in line_lower or 'adw id' in line_lower:
            continue

        # Skip model_set lines
        if 'model_set' in line_lower or 'model set' in line_lower:
            continue
    ...
    # If the prompt is empty, return the original text
    if not prompt:
        return text.strip()
