...
def emit_event(event_type: str, data: dict, adw_id: str, issue_number: str = None):
    ...
    context = {
        "workflow": "adw_plan_implement_update_task",
        "adw_id": adw_id,
...
def main(
    ...
    emit_event("workflow.started", {
        "workflow_stage": "plan_implement_update",
    ...
    try:
        ...
        if plan_response.success:
            ...
            emit_event("workflow.completed", {
                "workflow_stage": "planning",
                "task_description": f" - Created plan for: {task}"
            }, adw_id, issue_number)
        ...
        if issue_number:
            plan_summary_data["issue_number"] = issue_number

        with open(plan_summary_path, "w") as f:
            json.dump(plan_summary_data, f, indent=2)
        ...
        if workflow_success and plan_path:
            ...
            emit_event("workflow.started", {
                ...
                "task_description": f" - Implementing: {task}"
            }, adw_id, issue_number)
            ...
            if implement_response.success:
                ...
                emit_event("workflow.completed", {
                    "workflow_stage": "implementation",
                    "task_description": f" - Implemented: {task}"
                }, adw_id, issue_number)
