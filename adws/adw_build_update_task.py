...
def emit_event(event_type: str, data: dict, adw_id: str, issue_number: str = None):
    ...
    context = {
        "workflow": "adw_build_update_task",
        "adw_id": adw_id,
...
def main(
    ...
    try:
        ...
        emit_event("workflow.started", {
            ...
            "task_description": f" - Building: {task}"
        }, adw_id, issue_number)
        ...
        if build_response.success:
            ...
            emit_event("workflow.error", {"error_message": error_message}, adw_id, issue_number)
        ...
        if issue_number:
            build_summary_data["issue_number"] = issue_number

        with open(build_summary_path, "w") as f:
            json.dump(build_summary_data, f, indent=2)
        ...
        if issue_number:
            update_summary_data["issue_number"] = issue_number

        with open(update_summary_path, "w") as f:
            json.dump(update_summary_data, f, indent=2)
        ...
        if workflow_success:
            ...
            emit_event("workflow.completed", {
                "workflow_stage": "build_update",
                "task_description": f" - Successfully completed: {task}"
            }, adw_id, issue_number)
            ...
            emit_event("workflow.error", {
                "error_message": f"Workflow completed with errors: {error_message}"
            }, adw_id, issue_number)
