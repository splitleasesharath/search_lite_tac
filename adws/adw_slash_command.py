...
def main(
    ...
    emit_event("workflow.started", {
        "workflow_stage": "slash_command_execution",
        "task_description": f" - {slash_command} {' '.join(args) if args else ''}"
    }, adw_id, issue_number)
    ...
    try:
        ...
        if response.success:
            ...
            emit_event("workflow.completed", {
                "workflow_stage": "slash_command_execution",
                "task_description": f" - Successfully executed: {slash_command}"
            }, adw_id, issue_number)
            ...
            emit_event("workflow.error", {
                "error_message": f"Slash command execution failed: {slash_command}"
            }, adw_id, issue_number)
