...
from adw_modules.workflow_ops import extract_adw_info, extract_prompt_from_issue_body, AVAILABLE_ADW_WORKFLOWS
...
async def github_webhook(request: Request):
    ...
    try:
        ...
        if workflow:
            ...
            if workflow == "adw_chore_implement":
                # Extract the prompt from the issue body
                prompt = extract_prompt_from_issue_body(content_to_check, workflow)
                cmd = [
                    "uv",
                    "run",
                    trigger_script,
                ...
                # Default command format for other workflows
